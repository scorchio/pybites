...
class DB:
    ...
    def insert(self, table: str, values: List[Tuple]):
        ...
        if len(values) != num_columns_in_schema:
            raise SchemaError(f"Table {table} expects items with {num_columns_in_schema} values.")
        schema = {column: col_type for column, col_type in self.table_schemas[table]}
        for column, value in values:
            if not isinstance(value, schema[column].value):
                raise SchemaError(f"Column {column} expects values of type {schema[column].name}.")
